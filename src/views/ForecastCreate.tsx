...
const Label: React.FC<LabelProps> = ({ title, children, className }) => (
  <label htmlFor={title} className={`mb-2 font-medium capitalize ${className}`}>{children || title}</label>
);

const Input: React.FC<IInputProps> = ({ id, min, max, value, onChange, ...rest }) => (
  <input
...
const ForecastCreate: React.FC = observer(() => {
  ...
  return (
    <div>
      ...
      <div className='mt-20 text-gray-400'>
        ...
        <p>Cards with the same or close coordinates will be updated</p>
      </div>
    </div>
  );
});
